---
title: "Desafio 1 - Elementos de Aprendizado de Máquina"
warning: FALSE
echo: FALSE
---
...
tabela_influencia %>%
  kable(
    digits = 3,
    caption = "Tabela 1: Diagnóstico de Observações Influentes (Top 10)",
    col.names = c("ID", "Dist. Cook", "Leverage (Hat)", "Resíduo Std", 
                  "DFB Intercepto", "DFB Log(Rede)", "DFB Log(Chi)", "DFB Log(Pnt)")
  )
```
...
dados_refinado <- dados %>%
  mutate(
    dummy_id40 = ifelse(row_number() == 40, 1, 0), # O outlier de erro
    dummy_id50 = ifelse(row_number() == 50, 1, 0)  # O outlier de alavancagem (PNT zero)
  )
